...
func (d *daemon) runTriageReads(reads []readRequest) string {
	...
	for _, r := range reads {
		chatID, _, err := d.resolveChat(r.Target)
		if err != nil {
		...
		fmt.Fprintf(&b, "Chat %s (chat_id=%d), last %d message(s):\n", r.Target, chatID, len(history))
		downloads := 0
		for i := len(history) - 1; i >= 0; i-- { // history is newest-first
			dl := r.Files && downloads < maxReadDownloads
...
func (d *daemon) startTriageReply(st *userState) {
...
func (d *daemon) resetTriageBrain(st *userState) {
...
func (d *daemon) sendToRecipient(rec Recipient, body string) string {
	var err error
	switch rec.Source {
	case "wa":
		if !d.settings.WhatsApp.Enabled {
			...
			err = whatsapp.Send(d.settings.WhatsApp.Socket, rec.WAChat, body)
		}
	default:
...
func (d *daemon) sendFileTo(byIndex map[int]Recipient, target, path, caption string) string {
	...
	if idx, err := strconv.Atoi(target); err == nil {
		if rec, ok := byIndex[idx]; ok {
			if rec.Source == "wa" {
				...
				if err := whatsapp.SendFile(d.settings.WhatsApp.Socket, rec.WAChat, full, caption); err != nil {
...
func (d *daemon) handleTriageReplyOutput(chatID int64, recipients []Recipient, text string) {
	...
	for _, line := range strings.Split(text, "\n") {
		...
		if em := errandDispatch.FindStringSubmatch(trimmed); em != nil {
			spec, err := parseErrandStart(em[1])
			if err != nil {
				confirmations = append(confirmations, "⚠️ "+err.Error())
				continue
			}
			msg, err := d.startErrand(spec)
			if err != nil {
				confirmations = append(confirmations, "⚠️ Couldn't start errand: "+err.Error())
				continue
			}
