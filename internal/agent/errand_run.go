...
func (d *daemon) kickErrand(e *Errand) {
	if e.AutoStart {
...
func (d *daemon) approveErrand(e *Errand, tweak string) {
...
func (d *daemon) feedErrand(e *Errand, reply, mediaPath string) {
	d.mu.Lock()
...
func (d *daemon) driveErrandAsync(e *Errand, prompt string) {
	d.mu.Lock()
	if e.busy {
...
func (d *daemon) driveErrand(e *Errand, prompt string) {
	defer func() {
		if r := recover(); r != nil {
...
func (d *daemon) errandStatus(e *Errand) string {
	d.mu.Lock()
	defer d.mu.Unlock()
	return e.Status
}
...
func (d *daemon) runInterviewTurn(e *Errand, prompt string) (handoff bool) {
	...
	// RoleRead + non-writable staging = plan mode: the interviewer cannot write
	// files or run commands. Its only persistent output is the one file below.
	res, err := RunAgent(backend, staging, send, e.InterviewSessionID, RoleRead, false)
	if res.SessionID != "" {
...
func (d *daemon) applyInterviewDirectives(e *Errand, text string) (handoff bool) {
...
func (d *daemon) runWorkerTurn(e *Errand, prompt string) (done bool) {
	...
	// RoleRead + writable staging = acceptEdits in the scratch dir: the producer
	// can author the deliverable and read the collected-info file, but has no
	// auto-approved shell beyond that.
	res, err := RunAgent(backend, staging, send, e.WorkerSessionID, RoleRead, true)
	if res.SessionID != "" {
...
func (d *daemon) applyWorkerDirectives(e *Errand, text string) (done bool) {
...
func (d *daemon) ensureInterviewFile(e *Errand) {
	if e.InterviewFile != "" {
		return
	}
...
func (d *daemon) writeInterviewFile(e *Errand, content string) {
	d.ensureInterviewFile(e)
	if e.InterviewFile == "" {
		return
	}
...
func (d *daemon) beginProducing(e *Errand) {
	d.ensureInterviewFile(e)
...
func (d *daemon) errandTranscriptDoc(e *Errand) string {
	d.mu.Lock()
...
func (d *daemon) finishErrand(e *Errand, summary string) {
...
func (d *daemon) setErrandStatus(e *Errand, status string) {
	d.mu.Lock()
	e.Status = status
	d.mu.Unlock()
	_ = SaveErrand(e)
...
func (d *daemon) deliverToOwner(e *Errand, path, caption string) error {
...
func (d *daemon) sendToErrandTarget(e *Errand, body string) error {
	if e.Source == "wa" {
		if !d.settings.WhatsApp.Enabled {
			return fmt.Errorf("WhatsApp is disabled")
		}
		return whatsapp.Send(d.settings.WhatsApp.Socket, e.WAChat, body)
	}
	return d.sendErr(e.TGChat, body)
}

// sendFileToErrandTarget uploads a local file to the errand's contact.
func (d *daemon) sendFileToErrandTarget(e *Errand, path, caption string) error {
	if _, err := os.Stat(path); err != nil {
		return err
	}
	if e.Source == "wa" {
